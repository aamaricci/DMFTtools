...
function i3_check_convergence_relative(Xnew,eps,N1,N2,id,file,index,total,strict,oerr,reset,extend) result(convergence)
  integer,intent(in)                           :: Xnew(:,:,:,:)
  real(8)                        :: eps
  real(8),optional                             :: oerr
  logical,optional                             :: reset, extend
  ...
  integer,optional                             :: id,index,total
  integer                                      :: id_,index_,total_
  integer                                      :: i,j,k,Msize1,Msize2,Msize3,Msum
  logical                                      :: convergence  
  real(8)                                      :: error(2),err
  real(8),dimension(size(Xnew,1),size(Xnew,2),size(Xnew,3)) :: M,S,Verror
  integer,save,allocatable                     :: Xold(:,:,:,:,:)
  integer,save                                 :: success=0,check=1
  character(len=2)                             :: label
  logical,optional                             :: strict
  ...
  total_=1;if(present(total))total_=total
  index_=1;if(present(index))index_=index
  Msize1=size(Xnew,1);Msize2=size(Xnew,2);Msize3=size(Xnew,3);Msum=size(Xnew,4)
  if(.not.allocated(Xold))then
     allocate(Xold(total_,Msize1,Msize2,Msize3,Msum))
     Xold=0.d0
  endif
  S=0.d0 ; M=0.d0 ; Verror=0.d0
  do i=1,Msum
  ...
  if(strict_)err=error(1)
  Xold(index_,:,:,:,:)=Xnew
  include "error_write_file_dim3.f90"
  if(err < eps)then
     success=success+1
  else
     success=0
  ...
  convergence=.false.
  include "error_test_convergence.f90"
  include "error_msg_dim3.f90"
  if(present(oerr))oerr=err
  check=check+1
end function i3_check_convergence_relative
...
function d3_check_convergence_relative(Xnew,eps,N1,N2,id,file,index,total,strict,oerr,reset,extend) result(convergence)
  real(8),intent(in)                           :: Xnew(:,:,:,:)
  real(8)                        :: eps
  real(8),optional                             :: oerr
  logical,optional                             :: reset, extend
  ...
  if(present(oerr))oerr=err
  check=check+1
end function d3_check_convergence_relative
...
function z3_check_convergence_relative(Xnew,eps,N1,N2,id,file,index,total,strict,oerr,reset,extend) result(convergence)
  complex(8),intent(in)                        :: Xnew(:,:,:,:)
  real(8)                        :: eps
  real(8),optional                             :: oerr
  logical,optional                             :: reset, extend
  ...
  if(present(oerr))oerr=err
  check=check+1
end function z3_check_convergence_relative
