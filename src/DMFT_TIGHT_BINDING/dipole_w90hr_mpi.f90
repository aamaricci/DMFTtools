  subroutine dipole_t2g_LDA_mpi(MpiComm,Rx,Ry,Rz,Ruc,w90_file,Norb,Nlat,outputfile,Nr,varfact,t_thresh_)
   implicit none
   ...
   real(8),allocatable                          ::   dipole(:,:),dipole_aux(:,:)
   ...
   enddo
   read(106,*)
   if(mpi_master)write(*,'(A)')"  degen skipped"
   !
   !
   ...
   do ndx = 1+mpi_rank, totIntegrals, mpi_size
      ...
      enddo
      !
      dipole_aux(vecndx,1) = Dx
      dipole_aux(vecndx,2) = Dy
      dipole_aux(vecndx,3) = Dz
      !
   enddo
   call Mpi_AllReduce(dipole_aux,dipole, size(dipole), MPI_Double, MPI_Sum, MpiComm, mpi_ierr)
