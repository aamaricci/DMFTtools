...
!----------------------------------------------------------------------------------------
subroutine dmft_kinetic_energy_normal_main_mpi(MpiComm,Hk,Wtk,Sigma,Ekin,Eloc)
  integer                                     :: MpiComm
  ...
  do ik=1,Lk
     ...
     do i=1+mpi_rank,Liw,mpi_size
        ...
        do iso=1,Nso
           H0tmp(iso) = H0tmp(iso) + Wtk(ik)*Ck(iso,iso)
           Hltmp(iso) = Hltmp(iso) + Wtk(ik)*Dk(iso,iso)
        enddo
     enddo
     if(mpi_master)call eta(ik,Lk)
  enddo
  call AllReduce_MPI(MpiComm,H0tmp,H0)
  call AllReduce_MPI(MpiComm,Hltmp,Hl)
  ...
  !
  deallocate(wm)
end subroutine dmft_kinetic_energy_normal_main_mpi



subroutine dmft_kinetic_energy_normal_dos_mpi(MpiComm,Ebands,Dbands,Hloc,Sigma,Ekin,Eloc)
  integer                                                     :: MpiComm
  ...
  call assert_shape(Sigma,[Nso,Nso,Liw],"dmft_kinetic_energy_normal_dos_mpi","Sigma")
  !
  ...
  !
  deallocate(wm)
end subroutine dmft_kinetic_energy_normal_dos_mpi
...
subroutine dmft_kinetic_energy_normal_lattice_mpi(MpiComm,Hk,Wtk,Sigma,Ekin,Eloc)
  integer                                                         :: MpiComm
  ...
  Liw  = size(Sigma,4)
  !Testing:
  if(Nso/=Norb*Nspin)stop "dmft_kinetic_energy_normal_lattice_mpi: Nso != Norb*Nspin [from Sigma]"
  call assert_shape(Hk,[Nlat*Nso,Nlso,Lk],"dmft_kinetic_energy_normal_lattice_mpi","Hk") !implcitly test that Nlat*Nso=Nlso
  call assert_shape(Sigma,[Nlat,Nso,Nso,Liw],"dmft_kinetic_energy_normal_lattice_mpi","Sigma")
  !
  !Allocate and setup the Matsubara freq.
  if(allocated(wm))deallocate(wm);allocate(wm(Liw))
  ...
  do ik=1,Lk
     ...
     do i=1+mpi_rank,Liw,mpi_size
        ...
        Dk = matmul(Hloc,Gk(:,:) - Tk)
        do is=1,Nlso
           H0tmp(is) = H0tmp(is) + Wtk(ik)*Ck(is,is)
           Hltmp(is) = Hltmp(is) + Wtk(ik)*Dk(is,is)
        enddo
     enddo
     if(mpi_master)call eta(ik,Lk)
  enddo
  ...
  !
  deallocate(wm)
end subroutine dmft_kinetic_energy_normal_lattice_mpi
