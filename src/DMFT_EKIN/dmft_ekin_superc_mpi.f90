subroutine dmft_kinetic_energy_superc_main_mpi(MpiComm,Hk,Wtk,Sigma,Self,Ekin,Eloc)
  integer                                         :: MpiComm
  ...
  do ik=1,Lk
     ...
     do i=1+mpi_rank,Liw,mpi_size
        ...
        do is=1,Nso
           H0tmp(is) = H0tmp(is) + Wtk(ik)*Bk(is,is)
           Hltmp(is) = Hltmp(is) + Wtk(ik)*Ck(is,is)
        enddo
     enddo
  ...
  !
  deallocate(wm)
end subroutine dmft_kinetic_energy_superc_main_mpi


!
!DOS
subroutine dmft_kinetic_energy_superc_dos_mpi(MpiComm,Ebands,Dbands,Hloc,Sigma,Self,Ekin,Eloc)
  integer                                                     :: MpiComm
...
subroutine dmft_kinetic_energy_superc_lattice_mpi(MpiComm,Hk,Wtk,Sigma,Self,Ekin,Eloc)
  ...
  !
  deallocate(wm)
end subroutine dmft_kinetic_energy_superc_lattice_mpi
