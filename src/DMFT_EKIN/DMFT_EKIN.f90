...
contains 
  ...
  subroutine dmft_kinetic_energy_normal_Nso_main(Hk,Sigma,Ekin,Eloc)
    ...
    do ik=1,Lk
       ...
       do iso=1,Nso
          Tail0(iso) = Tail0(iso) + 0.5d0*Ak(iso,iso)
          Tail1(iso) = Tail1(iso) + 0.25d0*Ck(iso,iso)
          Lail0(iso) = Lail0(iso) + 0.5d0*Hloc(iso,iso)
          Lail1(iso) = Lail1(iso) + 0.25d0*Dk(iso,iso)
       enddo
    enddo
  ...
  subroutine dmft_kinetic_energy_normal_NN(Hk,Sigma,Ekin,Eloc)
    ...
    do ispin=1,Nspin
       ...
       enddo
    enddo
    call dmft_kinetic_energy_normal_main(Hk,Sigma_,Ekin_,Eloc_)
    if(present(Ekin))Ekin=Ekin_
    if(present(Eloc))Eloc=Eloc_
  ...
  subroutine dmft_kinetic_energy_normal_NN_lattice(Hk,Sigma,Ekin,Eloc)
    ...
    do ispin=1,Nspin
       ...
       enddo
    enddo
    call dmft_kinetic_energy_normal_lattice(Hk,Sigma_,Ekin_,Eloc_)
    if(present(Ekin))Ekin=Ekin_
    if(present(Eloc))Eloc=Eloc_
  ...
  subroutine dmft_kinetic_energy_superc_NN(Hk,Sigma,Self,Ekin,Eloc)
    ...
    do ispin=1,Nspin
       ...
       enddo
    enddo
    call dmft_kinetic_energy_superc_main(Hk,Sigma_,Self_,Ekin_,Eloc_)
    if(present(Ekin))Ekin=Ekin_
    if(present(Eloc))Eloc=Eloc_
  ...
  subroutine dmft_kinetic_energy_superc_NN_lattice(Hk,Sigma,Self,Ekin,Eloc)
    ...
    do ispin=1,Nspin
       ...
       enddo
    enddo
    call dmft_kinetic_energy_superc_lattice(Hk,Sigma_,Self_,Ekin_,Eloc_)
    if(present(Ekin))Ekin=Ekin_
    if(present(Eloc))Eloc=Eloc_
